...
void PacketControlSend(char *Packet, int Type)
{
	PacketHeader *p = (PacketHeader*)Packet;
	MOB *player = (MOB*)GetMobFromIndex(p->ClientID);
	int addr_bclient = p->ClientID * 0x0410;
...
void __stdcall Hooks::PacketControlFix(unsigned char* szBuffer, INT32 FixClientID)
{
	...
	switch(Header->PacketId)
	{
			...
			case 0x366://packet que derrubava o servidor
				MountSRACAO(Header->ClientID);
				MountExpira(Header->ClientID);
				if(Header->PacketId != 0x366){
					if(!FixClientID){
						Header->PacketId = NULL;
						SendClientMsg(FixClientID, "Wait a Moment");
						CloseUser(FixClientID);
					}
				}
				break;
		default:
			if(Header->PacketId != 0x20D && Header->PacketId != 0xFDE && Header->PacketId != 0x3A0)
			{
