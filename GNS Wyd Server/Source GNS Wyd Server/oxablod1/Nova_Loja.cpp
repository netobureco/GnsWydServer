...
void NPCNovaLoja(BYTE *m_PacketBuffer)
{
	...
	sprintf(local_item,"DataBase\\Loja_Nova\\Itens\\%d.txt",npc->Inventory[pak->SellSlot].Index);
	sprintf(local_cash,"DataBase\\Loja_Nova\\Gold\\[%s].txt",vLogin(clientid));
	{
		...
		if(arq != NULL)
		{
			...
			if(arq2 != NULL)
			{
				...
				{
					fclose(arq2);
					sprintf(msg,"Necessário [%d] Gold Loja.",preco);
			...
			{
				sprintf(msg,"Para Comprar Esse Item,Nescessario [%d] Gods Coins.",preco);
				SendClientMsg(clientid,msg);
				ZeroMemory(m_PacketBuffer, *(short*)m_PacketBuffer);
				return;
			}
